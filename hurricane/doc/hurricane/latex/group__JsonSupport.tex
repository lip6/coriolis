\hypertarget{group__JsonSupport}{\section{J\-S\-O\-N Support}
...
Instead of creating one more ad-\/hoc format, we just dump the \hyperlink{classHurricane_1_1DataBase}{Data\-Base} objects in a mirror like way in J\-S\-O\-N format.
...
For P\-O\-D types, four overloads of {\ttfamily json\-Writer()} are defined\-:
...
Note that through those three overloads we only provides support for pointers to object. The driving mechanism is designed in such a way that passing arguments by value is not supported for non-\/\-P\-O\-D types. Trying to do so will result in an unsupported message inside the generated J\-S\-O\-N file.\hypertarget{group__JsonSupport_secJsonDriverDBo}{}\subsubsection{D\-Bos Special Case}\label{group__JsonSupport_secJsonDriverDBo}
For \hyperlink{classHurricane_1_1DBo}{D\-Bo} objects, a complete parallel hierarchy of Json\-Objects mimicking the one of D\-Bos has been implemented. The {\ttfamily to\-Json()} function is implemented in the \hyperlink{classHurricane_1_1DBo}{D\-Bo} base object, and the derived classes must implement the following virtual functions\-:
...
The J\-S\-O\-N driver functions is splitted in two parts\-:
\begin{DoxyItemize}
...
The additionnal {\ttfamily to\-Json\-Signature()} method provide the signature for an \hyperlink{classHurricane_1_1Entity}{Entity} which is used by an occurrence. The signature of an occurrence is needed when we create a J\-S\-O\-N for a \hyperlink{classHurricane_1_1Cell}{Cell} only. In that case we cannot directly save the transhierarchical informations, so we need a way to characterize the deep \hyperlink{classHurricane_1_1Entity}{Entity} (which is not part of the saved \hyperlink{classHurricane_1_1Cell}{Cell}). Most of the time, the signature is the scalar attributes of the occurrenced object, it is far from foolproof, but it will do for now.\hypertarget{group__JsonSupport_secJsonParser}{}\subsection{J\-S\-O\-N Parser Support}\label{group__JsonSupport_secJsonParser}
To enable J\-S\-O\-N parsing support for an object, say \hyperlink{classHurricane_1_1Point}{Point}, an associated {\ttfamily Json\-Point} class must be created. This class must be derived (directly or not) from \hyperlink{classHurricane_1_1JsonObject}{Json\-Object}. It must implement one static functions and four methods, as shown below.
...
The constructor has to declare requirements, attributes, and collections needed to build the \hyperlink{classHurricane_1_1DataBase}{Data\-Base} object. Note the the requirements are not part of the objects but only needed to build it.
...
\{
  ...
  Point point ( \hyperlink{group__DbUGroup_gaec69d65ec1651c2feea24c5931f4580b}{DbU::fromDb}(get<int64\_t>(stack,\textcolor{stringliteral}{"\_x"}))
              , \hyperlink{group__DbUGroup_gaec69d65ec1651c2feea24c5931f4580b}{DbU::fromDb}(get<int64\_t>(stack,\textcolor{stringliteral}{"\_y"})) );
  update( stack, point );
\}
\end{DoxyCode}
\hypertarget{group__JsonSupport_secJsonArray}{}\subsubsection{J\-S\-O\-N Array}\label{group__JsonSupport_secJsonArray}
J\-S\-O\-N array are not translated into containers of any kind. They are simply ignored (from the stack point of view). Objects in array comes from a great variety of containers including \hyperlink{classHurricane_1_1Collection}{Hurricane\-::\-Collection}, in almost all cases, their very constructors are responsibles for inserting the object in the relevant container/collection. So there is no need to build a mechanism to keep track of all the objects in an array through a temporary container.

The corollary is that an object in an array must be able to extract the relevant context information from the stack. Say, if we are in an array of components, they must belong to a \hyperlink{classHurricane_1_1Net}{Net}, which must be present in the stack with a key \char`\"{}.\-Net\char`\"{}.\hypertarget{group__JsonSupport_secJsonStack}{}\subsubsection{Parser Stack}\label{group__JsonSupport_secJsonStack}
While performing the parsing, the parser maintain a stack (\hyperlink{classHurricane_1_1JsonStack}{Json\-Stack}) containing\-:
\begin{DoxyItemize}
\item The stack of attributes translateds, but not consumeds by the parser and the objects currently openeds (that is, which parsing is not completed). This stack contains P\-O\-D or \hyperlink{namespaceHurricane}{Hurricane} objects (value or pointer). It is a vector of pair {\ttfamily }(key,value) where the {\itshape key} is either the attribute name ({\ttfamily \-\_\-bounding\-Box}, {\ttfamily \-\_\-x\-Min}, {\ttfamily \-\_\-master\-Cell}, ...) or the class name ({\ttfamily }.\hyperlink{classHurricane_1_1Net}{Net}, {\ttfamily }.\hyperlink{classHurricane_1_1Cell}{Cell}, ...).
\item The stack of J\-S\-O\-N objects currently openeds, thoses objects are all derived classes of \hyperlink{classHurricane_1_1JsonObject}{Json\-Object}.
