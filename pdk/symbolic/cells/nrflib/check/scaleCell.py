#!/usr/bin/env python
# -*- coding: utf-8; explicit-buffer-name: "scaleCell.py<nrflib>" -*-
    ...
    for net in sourceCell.getNets():
      ...
      for component in net.getComponents():
        ...
        if isinstance(component,Contact):
          ...
          if component.getLayer().getName() == 'CONT_BODY_N':
            if component.getY() == DbU.fromLambda(47.0):
              print '[INFO] Shifted down CONT_BODY_N contact.'
              print '       %s' % component
              custom_dY = DbU.fromLambda(-1.0)
          
          if component.getLayer().getName() == 'CONT_BODY_N':
            if component.getY() == DbU.fromLambda(3.0):
              print '[INFO] Shifted up CONT_BODY_N contact.'
              print '       %s' % component
              custom_dY = DbU.fromLambda(1.0)
         ...
         # are already scaled (to the minimal size).
          dupComponent = Contact.create( scaledNet
                                       ...
                                       , component.getY     ()*scale + custom_dY
          ...
          if sourceCell.getName() == 'rf_fifo_ptset' or sourceCell.getName() == 'rf_fifo_ptreset':
            if component.getLayer().getName() == 'BLOCKAGE2':
              if     component.getSourceX() == DbU.fromLambda(10.0) \
                 and component.getTargetX() == DbU.fromLambda(45.0) \
                 and component.getY()       == DbU.fromLambda(20.0) \
                 and component.getWidth()   == DbU.fromLambda( 2.0):
                print '[INFO] Removed METAL2 blockage from "%s".' % sourceCell
                print '       %s' % component
                scaledNet.destroy()
                continue

          if sourceCell.getName() == 'rf_inmux_buf_2':
            if component.getLayer().getName() == 'METAL1':
              if     component.getSourceX() == DbU.fromLambda(27.0) \
                 and component.getTargetX() == DbU.fromLambda(39.0) \
                 and component.getY()       == DbU.fromLambda(90.0) \
                 and component.getWidth()   == DbU.fromLambda( 2.0):
                print '[INFO] Enlarge METAL1 to avoid notch.'
                print '       %s' % component
                custom_dW = DbU.fromLambda(1.0)

          if sourceCell.getName() == 'rf_fifo_clock':
            if component.getLayer().getName() == 'METAL1':
              if     component.getSourceX() == DbU.fromLambda(32.0) \
                 and component.getTargetX() == DbU.fromLambda(35.0) \
                 and component.getY()       == DbU.fromLambda(60.0) \
                 and component.getWidth()   == DbU.fromLambda( 2.0):
                print '[INFO] Enlarge METAL1 to avoid notch.'
                print '       %s' % component
                custom_dW = DbU.fromLambda(1.0)

          if component.getLayer().getName() == 'NTIE':
            if component.getY() == DbU.fromLambda(47.0):
              print '[INFO] Shifted down NTIE segment.'
              print '       %s' % component
              custom_dY = DbU.fromLambda(-1.0)
          
          if component.getLayer().getName() == 'PTIE':
            if component.getY() == DbU.fromLambda(3.0):
              print '[INFO] Shifted up PTIE segment.'
              print '       %s' % component
              custom_dY = DbU.fromLambda(1.0)
          ...
          if component.getLayer().getName() == 'METAL1':
            if component.getWidth() == DbU.fromLambda(2.0):
