...
def _routing ():
    ...
    rg.addLayerGauge(
        RoutingLayerGauge.create( tech.getLayer( 'METAL1' )         # metal
                                ...
                                , l(5.0)                           # track pitch
                                , l( 2.0)                           # wire width
                                , l( 2.0)                           # perpandicular wire width
                                , l( 1.0)                           # VIA side
                                , l( 3.5) ))                        # obstacle dW
    rg.addLayerGauge(
    ...
    with CfgCache(priority=Cfg.Parameter.Priority.ConfigurationFile) as cfg:
        ...
        cfg.etesian.latchUpDistance = l(1000.0)
       ...
       #cfg.etesian.antennaMaxWL = u(250.0)
        ...
        cfg.anabatic.routingGauge = 'lsxlib'
        ...
        cfg.katana.hTracksReservedLocal = 11
        cfg.katana.hTracksReservedLocal = [0, 20]
        cfg.katana.vTracksReservedMin   = 4
        cfg.katana.vTracksReservedLocal = 11
        ...
        cfg.block.spareSide = 10*l(50.0)
        cfg.spares.buffer = 'buf_x8'
        cfg.spares.maxSinks = 31


def _loadLsxlib ( cellsTop ):
    """
    ...
    env.addSYSTEM_LIBRARY ( library=(cellsTop / 'lsxlib').as_posix(), mode=Environment.Append )

def setup ( cellsTop ):

    _routing()
    _loadLsxlib( cellsTop )
