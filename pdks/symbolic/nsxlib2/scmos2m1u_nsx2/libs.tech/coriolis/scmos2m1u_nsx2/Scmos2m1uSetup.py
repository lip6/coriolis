...
__all__ = [ 'Where', 'setupScmos2m1u_nsx2 ' ]
...
def setupScmos2m1u_nsx2 ( checkToolkit=None ):
    Where( checkToolkit )
    ShellEnv().export()

    pdkDir          = Where.checkToolkit / 'dks' / 'scmos2m1u_nsx2' / 'libs.tech'
    coriolisTechDir = pdkDir / 'coriolis'
    if not pdkDir.is_dir():
        print( '[ERROR] technos.setupScmos2m1u_nsx2(): PDK directory do *not* exists:' )
...
#    kdrcRules = pdkDir / 'klayout' / 'drc' / 'drc_SCMOS.lydrc'
    ...
    from scmos2m1u_nsx2 import techno, nsxlib2
